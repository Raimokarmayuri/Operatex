...
import AddDowntime from "../MachineCard/Downtime";
import AddRejectPart from "../MachineCard/Rejectedpart";
import HourlyProductionChart from "../MachineCard/HourlyProdCount";
import {
  Container,
  Row,
  Col,
...
const Summary = () => {
  ...
  const {
    ...
    queryKey: ["oeeData", machineId],
    queryFn: () => fetchMachineOEEData(machineId),
  ...
  const { data: reworkQuantity = 0, isLoading: isReworkLoading } = useQuery({
    queryKey: ["reworkData", machineId],
    queryFn: () => fetchReworkData(machineId),
