...
const SetupApprovalQualityView = () => {
  ...
  const fetchSetupApprovalData = async (machineId, partId) => {
    try {
      ...
      // ✅ Build a lookup map from the qualityMeta response
      const qualityInfoMap = {};
      qualityMeta.data.forEach((q) => {
        qualityInfoMap[q.parameters] = {
          quality_part_count: q.quality_part_count || 0,
          boolean_expected_value: q.boolean_expected_value || false,
        };
      });
  
      // ✅ Filter the setup-approval record for the given machine, part, and 'Pending' status
      const filtered = res.data.find(
        (sa) =>
          sa.status === "Pending" &&
          parseInt(sa.machine_id) === parseInt(machineId) &&
      ...
      if (filtered) {
        ...
        // ✅ Map the parameters from setup-approval and attach boolean + quality part count
        const rows = filtered.parameters.map((param) => {
          const paramKey = param.parameter || param.name || "";
          const qualityInfo = qualityInfoMap[paramKey] || {};
  
          const count = qualityInfo.quality_part_count || 2;
          const inputs = param.inputs || [];
          const inputFields = Array.from(
            { length: count },
            (_, i) => inputs[5 + i] || ""
          ...
          return {
            ...
            quality_part_count: count,
            remark_by_quality: param.remark_by_quality || "",
            boolean_expected_value: Boolean(qualityInfo.boolean_expected_value), // ✅ FIXED
          };
        });
  
        setParameterRows(rows);
      } else {
        setSetupApprovalId(null);
        setParameterRows([]);
        setInfoMsg("✅ No pending setup approval found for selected machine and part.");
      }
    } catch (err) {
      console.error("Failed to fetch setup approvals", err);
      setSetupApprovalId(null);
      setParameterRows([]);
      setInfoMsg("⚠️ Error fetching setup approval data.");
    }
  };
    ...
    try {
      await axios.put(
        `${API_BASE_URL}/api/setup-approvals/${setupApprovalId}`,
        {
          ...
          remark_by_quality: remarkByQuality,
          status: "Completed" // ✅ added to mark it as completed
        }
  ...
  return (
    <div className="container3 p-3">
      <h4 className="mb-3 text-primary">Quality Approval</h4>
            ...
            <Table bordered hover responsive>
              <thead className="table-light">
                <tr>
                  ...
                  {[1, 2, 3, 4, 5].map((n) => (
                    <th className="text-primary" key={n}>Input {n}</th>
                  ))}
                  <th className="text-primary" colSpan={5}>Dynamic Inputs (Quality)</th>
                  <th className="text-primary">Remark by Quality</th>
                </tr>
              </thead>
              <tbody>
                {parameterRows.map((row, idx) => (
  ...
  <td key={i}>
    {row.boolean_expected_value ? (
      <Form.Select
        value={val}
        required
        onChange={(e) => handleDynamicInputChange(idx, i, e.target.value)}
      >
        <option value="">Select</option>
        <option value="OK">OK</option>
        <option value="Not OK">Not OK</option>
