  ...
  return (
    <div className="container3 mb-4" style={{ marginTop: "50px" }}>
      <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2 p-2 border rounded bg-light">
        <div className="fs-4 fw-bold" style={{ color: "#034694" }}>Part Master</div>
        <button className="btn btn-primary" onClick={() => setShowForm(true)}>+ Add New</button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="row ms-2" encType="multipart/form-data">
          ...
          <tbody>
            {parts.map(part => (
              <tr key={part.part_id}>
                ...
                <td>
                  <button className="btn btn-sm btn-outline-primary me-1" onClick={() => handleEdit(part)}>
                    <BsPencil />
                  </button>
                  <button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(part.part_id)}>
                    <BsTrash />
                  </button>
                </td>
              </tr>
